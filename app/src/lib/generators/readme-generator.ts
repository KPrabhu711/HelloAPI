...
export function generateReadme(spec: ApiSpec): string {
    ...
    // Title
    sections.push(`# ${spec.title} — Quickstart Guide\n`);
...
function renderCurlForEndpoint(spec: ApiSpec, ep: Endpoint): string {
    ...
    curl += `\n\`\`\`\n`;
    curl += `\nExpected: **${ep.responses[0]?.statusCode || '200'}** — ${ep.responses[0]?.description || 'Success'}\n`;
    return curl;
}
...
| **403** | Forbidden | Check permissions — your key may not have access to this resource |
...
| **500** | Server Error | Retry after a short delay — this is usually temporary |
`;
}
